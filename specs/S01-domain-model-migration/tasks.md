...
#### T-001: Package Migration for ShoppingCart
**Class: rewrite**
...
#### T-002: Package Migration for Product  
**Class: rewrite**
...
#### T-003: Package Migration for ShoppingCartItem
**Class: rewrite** 
...
#### T-004: Package Migration for Promotion
**Class: rewrite**
...
#### T-005: Legacy UI Surface Waiver
**Class: infer**
Explicitly waive legacy UI surface coverage since domain models are backend-only.

**UI Surface**: No direct UI surface exposure - domain models consumed only by service layer
**Waiver Rationale**: REST endpoints and UI contracts owned by S03; domain models provide backend data contracts only
**Finding Rule IDs**: springboot-web-to-quarkus-00000 (waived)

**Decided Design**:
- **File Mapping**: No UI files in scope - waiver only
- **API Contract**: ShoppingCart JSON contract maintained via @JsonIgnore, @JsonProperty annotations preserved
- **Backward Compatibility**: Serialization contracts maintained for CartEndpoint integration (S03)
- **Package**: com.demo.model.* (no UI packages)

**Coverage**: Domain model serialization contracts preserved for downstream REST endpoints
**Preservation**: ShoppingCart JSON contract maintained for CartEndpoint integration

**Verification**: UI surface coverage explicitly addressed via waiver
...
- Preserved via application.properties migration (owned by S03)
...
**Design Decisions**:
- External catalog endpoint configuration preserved for S03 ownership
...
**Rewrite Tasks**: T-001 through T-004 (package migration, file harvesting)
**Infer Tasks**: T-005 through T-010 (waivers, integration coverage, characterization tests)
...
**Waived Items**: 
- springboot-web-to-quarkus-00000 (T-005) - UI surface waived as backend-only domain models

**God Node Priority**: ShoppingCart (T-001, T-007) and Product (T-002, T-009) converted first with early characterization test coverage.
